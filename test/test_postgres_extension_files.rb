...
class TestPostgresExtensionFiles < Minitest::Test
  ...
  def test_makefile_links_vendored_zstd_archive_without_exposing_its_objects_to_pgxs
    ...
    assert_includes makefile, "SHLIB_LINK  += $(ZSTD_LIB)"
    refute_match(/^OBJS[^\n]*\$\(ZSTD_OBJS\)/, makefile)
  end

  def test_e2e_script_is_pinned_and_covers_the_view_path
    script = read("test/run_e2e.sh")

    assert_includes script, "postgres:17.9-bookworm@sha256:"
    assert_includes script, "--platform"
    assert_includes script, "CREATE EXTENSION multi_compress"
    assert_includes script, "CREATE VIEW app.events_readable"
    assert_includes script, "MCDB_PG_SO"
    assert_includes script, "install_postgres_build_tools"
    assert_includes script, "copy_postgres_build_tree"
    assert_includes script, "trap cleanup EXIT"
    refute_includes script, "MCDB_PG_CONFIG"
  end
end
